...
const ServiceDetailsEditor = ({
  ...
  const groupedSportsSubjects = useMemo(() => {
    ...
    return {
      ...
      crafts:   { title: t('filters.tutoring.crafts'),   items: sportsSubcategories.filter(s => inRange(s, 50, 59)) },
  ...
  const renderField = (field) => {
    ...
    if (!isEditMode) {
      if (field.type === 'json-array' || field.type === 'checkbox') {
        // Langues : afficher le texte libre à la place du marqueur "other"
        const displayArray = (field.name === 'languages' && Array.isArray(value))
          ? value.map(v => (v === 'other' && serviceDetails?.languages_other) ? serviceDetails.languages_other : v)
          : value;
        return (
          <div className="tags-list">
