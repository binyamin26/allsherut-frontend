...
const PhotographerPage = () => {
  ...
  const [locationFilter, setLocationFilter] = useLocationFilter('photographer');
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(t('services.photographer.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  const handleLocationChange = (newLocation) => setLocationFilter(newLocation);
  ...
  const handleReviewCreated = () => loadProviders();

  return (
    <div className="service-page photographer-page">
      <SEO
  title={t('services.photographer.pageTitle', SERVICE_PAGE_META.photographer.title)}
  description={t('services.photographer.desc', SERVICE_PAGE_META.photographer.description)}
  canonicalPath={buildServicePath(serviceTypeToKey('photographer'), 'he')}
  jsonLd={buildServicePageJsonLd({ serviceId: 'photographer', name: t('services.photographer.pageTitle', SERVICE_PAGE_META.photographer.title), description: t('services.photographer.desc', SERVICE_PAGE_META.photographer.description), t })}
      />
      <ServiceBreadcrumb serviceId="photographer" />
      <section className="service-header">
        <div className="container">
          <div className="service-title-section">
            <div className="service-hero-icon">
              <img src={SERVICE_PAGE_META.photographer.heroImage}
                alt={SERVICE_PAGE_META.photographer.heroAlt} />
            </div>
            <h1 className="service-title">{t('services.photographer.pageTitle')}</h1>
          </div>
          <ServiceHeaderSubtitle serviceId="photographer" />
        </div>
      </section>
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
                <div className="loading-text">{t('services.photographer.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
                  <strong>{specialty ? filteredProviders.length : resultsCount}</strong> {t('services.photographer.found')}
          ...
          {loading ? (
            <div className="providers-loading">
              ...
              <p>{t('services.photographer.loading')}</p>
            </div>
          ) : error ? (
            ...
            <div className="no-results">
              <h3>{t('services.photographer.noResults')}</h3>
