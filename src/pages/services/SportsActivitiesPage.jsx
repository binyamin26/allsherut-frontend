...
const SportsActivitiesPage = () => {
  ...
  const [locationFilter, setLocationFilter] = useLocationFilter('sports_activities');
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(t('services.sports_activities.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  return (
    <div className="service-page tutoring-page">
      <SEO
  title={t('services.sports_activities.pageTitle', SERVICE_PAGE_META.sports_activities.title)}
  description={t('services.sports_activities.desc', SERVICE_PAGE_META.sports_activities.description)}
  canonicalPath={buildServicePath(serviceTypeToKey('sports_activities'), 'he')}
  jsonLd={buildServicePageJsonLd({ serviceId: 'sports_activities', name: t('services.sports_activities.pageTitle', SERVICE_PAGE_META.sports_activities.title), description: t('services.sports_activities.desc', SERVICE_PAGE_META.sports_activities.description), t })}
      />
      ...
      <section className="service-header">
        <div className="container">
          ...
          <ServiceHeaderSubtitle serviceId="sports_activities" />
        </div>
      </section>
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
                <div className="loading-text">{t('services.sports_activities.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
                  <strong>{specialty ? filteredProviders.length : resultsCount}</strong> {t('services.sports_activities.found')}
          ...
          {loading ? (
            <div className="providers-loading">
              ...
              <p>{t('services.sports_activities.loading')}</p>
            </div>
          ) : error ? (
            ...
            <div className="no-results">
              <h3>{t('services.sports_activities.noResults')}</h3>
