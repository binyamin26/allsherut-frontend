...
const CateringPage = () => {
  ...
  const [locationFilter, setLocationFilter] = useLocationFilter('catering');
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      if (response.success) {
       ...
       setError(t('services.catering.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  return (
    <div className="service-page catering-page">
      <SEO
  title={t('services.catering.pageTitle', SERVICE_PAGE_META.catering.title)}
  description={t('services.catering.desc', SERVICE_PAGE_META.catering.description)}
  canonicalPath={buildServicePath(serviceTypeToKey('catering'), 'he')}
  jsonLd={buildServicePageJsonLd({ serviceId: 'catering', name: t('services.catering.pageTitle', SERVICE_PAGE_META.catering.title), description: t('services.catering.desc', SERVICE_PAGE_META.catering.description), t })}
      />
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
             <div className="loading-text">{t('services.catering.searching')}</div>
              ) : error ? (
  ...
  {!locationFilter.neighborhood && locationFilter.city && <span> {t('common.in')} <strong style={{color:'#dc2626',fontSize:'1.15em'}}>{locationFilter.city}</strong></span>}
          ...
          {loading ? (
           ...
           <p>{t('services.catering.loading')}</p>
            </div>
          ) : error ? (
  ...
  />
            ...
            <div className="no-results">
              <h3>{t('services.catering.noResults')}</h3>
