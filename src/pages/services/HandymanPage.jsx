...
const HandymanPage = () => {
  ...
  const [locationFilter, setLocationFilter] = useLocationFilter('handyman');
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(t('services.handyman.loadError'));
        setProviders([]);
        setResultsCount(0);
      }
    } catch (error) {
  ...
  return (
    <div className="service-page handyman-page">
      <SEO
  title={t('services.handyman.pageTitle', SERVICE_PAGE_META.handyman.title)}
  description={t('services.handyman.desc', SERVICE_PAGE_META.handyman.description)}
  canonicalPath={buildServicePath(serviceTypeToKey('handyman'), 'he')}
  jsonLd={buildServicePageJsonLd({ serviceId: 'handyman', name: t('services.handyman.pageTitle', SERVICE_PAGE_META.handyman.title), description: t('services.handyman.desc', SERVICE_PAGE_META.handyman.description), t })}
      />
      ...
      <section className="service-header">
        <div className="container">
          ...
          <ServiceHeaderSubtitle serviceId="handyman" />
        </div>
      </section>
        ...
        <div className="results-container">
          <div className="results-summary">
            <div className="results-info">
              {loading ? (
                <div className="loading-text">{t('services.handyman.searching')}</div>
              ) : error ? (
                <div className="error-text">{error}</div>
              ) : (
                <div className="results-count">
                  <strong>{specialty ? filteredProviders.length : resultsCount}</strong> {t('services.handyman.found')}
          ...
          {loading ? (
            <div className="providers-loading">
              ...
              <p>{t('services.handyman.loading')}</p>
            </div>
          ) : error ? (
            ...
            <div className="no-results">
              <h3>{t('services.handyman.noResults')}</h3>
